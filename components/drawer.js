...
const DrawerComponent = ({setOpen,open}) => {
  ...
  const active="'text-lg font-bold text-center'"
  const inactive="'text-lg text-center'"
  ...
  return (
      <Drawer placement="right" onClose={()=>{setOpen(false)}} open={open}>
        <div className='flex flex-col justify-center items-center gap-5'>
            ...
            <Link href={"/services"}><p className={router.pathname=="/services"?active:inactive} onClick={()=>{setOpen(false)}}>Services</p></Link>
            <Link href={"/projects"}><p className={router.pathname=="/projects"?active:inactive} onClick={()=>{setOpen(false)}}>Projects</p></Link>
